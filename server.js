...
const DB_JASA_PATH = path.join(__dirname, 'database', 'admin', 'jasa.json');

app.post('/api/user/order-jasa', (req, res) => {
    if (!req.session || !req.session.user) {
        return res.status(401).json({ success: false, message: 'Silahkan login terlebih dahulu!' });
    }

    const { jasaId, emailKontak, formatOrder } = req.body;
    const usernameSession = req.session.user.username;

    if (!jasaId || !emailKontak || !formatOrder) {
        return res.status(400).json({ success: false, message: 'Data pesanan (Email dan Format Order) tidak boleh kosong.' });
    }

    try {
        if (!fs.existsSync(DB_JASA_PATH)) {
            return res.json({ success: false, message: 'Database jasa tidak ditemukan.' });
        }

        const jasaList = JSON.parse(fs.readFileSync(DB_JASA_PATH, 'utf8') || '[]');
        const jasaIndex = jasaList.findIndex(j => j.jasaId === jasaId);

        if (jasaIndex === -1) {
            return res.json({ success: false, message: 'Layanan jasa tidak ditemukan.' });
        }
        ...
        if (userIndex === -1) {
            return res.json({ success: false, message: 'Data user tidak valid.' });
        }

        const hargaJasa = parseInt(jasaList[jasaIndex].harga || 0);
        const saldoUser = parseInt(users[userIndex].saldo || 0);

        if (saldoUser < hargaJasa) {
            return res.json({ success: false, message: 'Saldo kamu tidak mencukupi untuk membeli jasa ini.' });
        }

        users[userIndex].saldo = saldoUser - hargaJasa;
        req.session.user.saldo = users[userIndex].saldo;
        fs.writeFileSync(USER_DB_PATH, JSON.stringify(users, null, 4), 'utf8');

        let adminEmail = 'hexaelitemarket@gmail.com'; 
        const adminData = users.find(u => u.role === 'admin' || u.role === 'owner');
        ...
        let orderData = JSON.parse(fs.readFileSync(ORDER_PO_PATH, 'utf8') || '[]');
        const invoiceBaru = "INV-JASA" + Date.now();
        
        orderData.push({
            invoiceId: invoiceBaru,
            username: usernameSession,
            email: emailKontak,
            productId: jasaId,
            productName: jasaList[jasaIndex].namaJasa,
            harga: hargaJasa,
            formatPesan: formatOrder,
            status: "Proses Jasa",
            date: new Date().toLocaleString('id-ID')
        });
        fs.writeFileSync(ORDER_PO_PATH, JSON.stringify(orderData, null, 4), 'utf8');
        ...
        const mailOptionsToAdmin = {
            from: '"Notifikasi Sistem Jasa" <hexaelitemarket@gmail.com>',
            to: adminEmail,
            subject: `Pesanan Jasa Baru: ${jasaList[jasaIndex].namaJasa}`,
            html: `
                <div style="font-family: sans-serif; padding: 20px; background-color: #0f1015; color: #ffffff;">
                    <h2 style="color: #dc2626; margin-bottom: 5px;">PESANAN JASA BARU MASUK!</h2>
                    <p style="color: #a1a1aa; font-size: 12px; margin-top: 0;">Ada user yang membeli layanan jasa kamu.</p>
                    
                    <table style="width: 100%; border-collapse: collapse; margin-top: 20px;">
                        <tr>
                            <td style="padding: 10px; border-bottom: 1px solid #27272a; color: #a1a1aa; width: 120px;">Invoice</td>
                            <td style="padding: 10px; border-bottom: 1px solid #27272a; font-family: monospace; font-weight: bold; color: #f97316;">${invoiceBaru}</td>
                        </tr>
                        <tr>
                            <td style="padding: 10px; border-bottom: 1px solid #27272a; color: #a1a1aa;">Layanan</td>
                            <td style="padding: 10px; border-bottom: 1px solid #27272a; font-weight: bold; text-transform: uppercase;">${jasaList[jasaIndex].namaJasa}</td>
                        </tr>
                        <tr>
                            <td style="padding: 10px; border-bottom: 1px solid #27272a; color: #a1a1aa;">Username</td>
                            <td style="padding: 10px; border-bottom: 1px solid #27272a; font-weight: bold;">${usernameSession}</td>
                        </tr>
                        <tr>
                            <td style="padding: 10px; border-bottom: 1px solid #27272a; color: #a1a1aa;">Email User</td>
                            <td style="padding: 10px; border-bottom: 1px solid #27272a; font-weight: bold; color: #3b82f6;">${emailKontak}</td>
                        </tr>
                    </table>

                    <div style="margin-top: 25px;">
                        <span style="font-size: 10px; font-weight: bold; color: #71717a; text-transform: uppercase; letter-spacing: 1px;">FORMAT ORDER DARI PEMBELI :</span>
                        <div style="background-color: #14151f; border: 1px solid #27272a; padding: 15px; border-radius: 12px; font-family: monospace; font-size: 13px; color: #10b981; margin-top: 8px; white-space: pre-wrap;">${formatOrder}</div>
                    </div>
                </div>
            `
        };
        ...
        if (typeof poTransporter !== 'undefined') {
            poTransporter.sendMail(mailOptionsToAdmin, (err) => {
                if (err) console.error("Gagal mengirim email notif ke admin:", err);
            });
        }

        res.json({ 
            success: true, 
            message: 'Pesanan jasa berhasil dibuat! Admin akan memproses orderan kamu berdasarkan format yang dikirim.' 
        });

    } catch (error) {
        console.error(error);
        res.status(500).json({ success: false, message: 'Gagal memproses pembelian jasa di server.' });
    }
});
...
app.get('/api/user/list-jasa', (req, res) => {
    try {
        ...
        // Hanya kirimkan jasa yang statusnya aktif (jika ada filter status)
        const aktifJasa = jasaList.filter(j => j.status !== 'Nonaktif');
        
        res.json({ success: true, data: aktifJasa });
    } catch (e) {
        console.error(e);
        res.status(500).json({ success: false, message: 'Internal Server Error' });
    }
